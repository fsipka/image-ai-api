...
const generationBodySchema = Joi.object({
  ...
  parameters: Joi.object({
    ...
    negativePrompt: Joi.string().max(500).optional().allow(''),
    style: Joi.string().default('photographic'),
    quality: Joi.number().integer().min(1).max(4).default(2),
    steps: Joi.number().integer().min(10).max(50).default(25),
    guidanceScale: Joi.number().min(1).max(20).default(7.5),
    seed: Joi.number().integer().optional().allow(null),
    width: Joi.number().integer().min(512).max(2048).default(1024),
    height: Joi.number().integer().min(512).max(2048).default(1024),
